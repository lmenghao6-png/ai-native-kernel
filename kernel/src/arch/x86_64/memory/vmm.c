...
static uint64_t table_flags(uint64_t page_flags) {
    return VMM_PAGE_WRITABLE | (page_flags & VMM_PAGE_USER) | 1;
}
...
static inline uint64_t *pdpt_entry(uint64_t *pml4e, uint64_t vaddr, uint64_t flags) {
    if (!(*pml4e & 1)) {
        uint64_t p = pmm_alloc(); if(!p) return 0;
        memset((void*)(p+hhdm),0,0x1000);
        *pml4e = p | table_flags(flags);
    }
    uint64_t *pdpt = (uint64_t *)((*pml4e & ~0xFFFULL) + hhdm);
    return &pdpt[(vaddr >> 30) & 0x1FF];
}
static inline uint64_t *pd_entry(uint64_t *pdpte, uint64_t vaddr, uint64_t flags) {
    if (!(*pdpte & 1)) {
        uint64_t p = pmm_alloc(); if(!p) return 0;
        memset((void*)(p+hhdm),0,0x1000);
        *pdpte = p | table_flags(flags);
    }
    uint64_t *pd = (uint64_t *)((*pdpte & ~0xFFFULL) + hhdm);
    return &pd[(vaddr >> 21) & 0x1FF];
}
static inline uint64_t *pt_entry(uint64_t *pde, uint64_t vaddr, uint64_t flags) {
    if (!(*pde & 1)) {
        uint64_t p = pmm_alloc(); if(!p) return 0;
        memset((void*)(p+hhdm),0,0x1000);
        *pde = p | table_flags(flags);
    }
...
bool vmm_map_at(uint64_t va, uint64_t pa, uint64_t flags) {
    if (!owned_cr3) return false;
    uint64_t *p4 = pml4_entry(owned_cr3, va);
    if (!p4) return false;
    uint64_t *p3 = pdpt_entry(p4, va, flags);
    if (!p3) return false;
    uint64_t *p2 = pd_entry(p3, va, flags);
    if (!p2) return false;
    uint64_t *p1 = pt_entry(p2, va, flags);
    if (!p1) return false;
