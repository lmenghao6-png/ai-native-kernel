...
void vfs_attach(vfs_node_t *p, vfs_node_t *c){c->parent=p;c->next=p->children;p->children=c;}
...
vfs_file_t *vfs_open(const char *path){vfs_node_t *n=vfs_lookup(path);if(!n||n->type!=VFS_FILE)return 0;vfs_file_t *f=kmalloc(sizeof(*f));f->node=n;f->offset=0;return f;}
int vfs_read(vfs_file_t *f, void *b, size_t s){if(!f||!f->node||!f->node->data)return-1;size_t r=f->node->size-f->offset;if(!r)return 0;size_t t=s<r?s:r;memcpy(b,f->node->data+f->offset,t);f->offset+=t;return(int)t;}
